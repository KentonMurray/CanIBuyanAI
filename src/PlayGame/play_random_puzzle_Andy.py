...
# Add Andy directory to path and change working directory for data/ paths
andy_dir = "/home/li126/CanIBuyanAI/Andy"
sys.path.insert(0, andy_dir)
import main
...
def computer_turn_Andy(showing, winnings, previous_guesses, turn):
  """
  main function
  ...
  previous_guesses_clean = [x for x in previous_guesses if x != "_"]

  #avoid guess taking too long
  if (len(previous_guesses_clean) >= 2):
    result = main.wof_solver(showing, previous_guesses_clean)  
  else:
    result = main.wof_solver("aeiou_", previous_guesses_clean)

  potential_guess = result["next_letter"]
  print(potential_guess)
  guess = potential_guess[0][0]



  for x in range(len(potential_guess)):
    #buy vowel when you can and when it is best chance
    if (potential_guess[x][0] not in list("AEIOU")) or winnings[(turn % 3)] >= 250:
      guess = potential_guess[x][0]
      #if there are no likely guesses, use letter frequencies
      if potential_guess[x][1] == -1:
        alphabet = "EARIOTNSLCUDPMHGBFYWKVXZJQ"
        found = False
        for y in alphabet:
          for z in potential_guess:
            if y == z[0]:
              guess = y
              found = True
            if (found):
  ...
  if (guess in "AEIOU" and winnings[(turn % 3)] < 250):
    guess = random.choice([x for x in "ABCDEFGHIJKLMNOPQRSTUVWXYZ" if ((x not in "AEIOU") and (x not in previous_guesses) and (x not in showing))])

  #calculate money and print details
  if is_vowel(guess):
    print("Andy bought:", guess)
    winnings[(turn % 3)] = winnings[(turn % 3)] - 250
  # Want to choose a consonant ... so spins wheel
  dollar = spin_wheel()
  if dollar == 0:
    print("Andy lost a turn")
    guess = "_"
  elif dollar == -1:
    print("Andy went backrupt")
    winnings[(turn % 3)] = 0
    guess = "_"
  else:
    print("Andy guessed:", guess)
...
def play_random_game(type_of_players):
  ...
  while showing != puzzle:
    ...
    elif type_of_player == "Andy":
      guess, dollar = computer_turn_Andy(showing, winnings, previous_guesses, turn)
...
if __name__ == '__main__':
  ...
  if len(type_of_players) != 3:
    ...
    type_of_players = ["Andy", "smart", "conservative"] # Updated default with smart players
    time.sleep(3)
